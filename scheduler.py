...
# добавить обработку времени? а не только дат
# датакласс для Notification
class Notification:
    DATE_FORMATS = [
        "%Y-%m-%d",  # 2025-10-28
        "%d/%m/%Y",  # 28/10/2025
    ...
    def __init__(self, user_id: str, notification: tuple[
        str, str | dt.date, str | None] = None):  # dict[str: str, str: str | dt.date, str: str | None]
    ...
    def run_pending(self):
        ...
        for notif in self.notifications:
            ...
            if send_at and send_at <= today:
                message = notif['message']
                if message:
                    print(f'INFO: Sending message: "{message}" with '
                          f'priority: {notif['priority']}, with datetime {send_at} '
                          f'to user with id: {notif['user_id']}')
                else:
...
notif1 = Notification("123", ("вот зачем я так усложнил", "2025-10-28", 'normal'))
# print(notif.date)  # 2025-10-28
notif2 = Notification("124", ("вот зачем я так усложнил", "2025-11-28"))
# print(notif.date)  # 2025-10-28
...
class Schedule:
    def __init__(self):
        pass  # больше не храним в памяти

    @staticmethod
    async def schedule(notif: Notification):
        ...
        print(f"Уведомление сохранено в БД для пользователя {notif.user_id}")

    @staticmethod
    async def run_pending():
        """отправляет все просроченные \ готовые уведомления из БД"""
        pending = await NotificationDB.get_pending_due()
        if not pending:
            print("INFO: Нет уведомлений для отправки")
            return

        print(f"INFO: Отправка {len(pending)} уведомлений")
        for row in pending:
            print(f'INFO: Sending message: "{row["message"]}" with '
                  f'priority: {row["priority"]}, date: {row["send_at"]} '
