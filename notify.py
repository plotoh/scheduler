...
class Notification:
    ...
    def __init__(self, user_id: str, notification: tuple[str, str|dt.date] = None):
        self.user_id = user_id
        self.message, self.send_at = None, None
        try:
    ...
    def get_notification(self, notif=None):
        if not (isinstance(notif, tuple) and len(notif) == 2):
        ...
        self.message, self.send_at = msg, parsed_date

    # def send_notification(self):
    #     today = dt.date.today()
    #     if self.send_at and self.send_at <= today:
    #         message = self.message
    #         if message:
    #             print(f'INFO: Sending message: {message} with date {self.send_at} to user with id: {self.user_id}')
    #             # return f'INFO: Sending message: {message} with date {self.date} to user with id: {self.user_id}'
    #     else:
    ...
    def run_pending(self):
        ...
        for notif in self.notifications:
            ...
            if send_at and send_at <= today:
                message = notif['message']
                if message:
                    print(f'INFO: Sending message: "{message}" with datetime {send_at} to user with id: {notif['user_id']}')
                else:
                    print("уведомления нет. не знаю почему. но мы разберемся. наверное")
            else:
                continue
...
notif1 = Notification("123", ("вот нахуя я так усложнил", "2025-10-28"))
